...
def main(out_features = 16, batch_size = 64, lr = 1e-4, epochs=1, max_pairs=8000, an_ratio = 0.5, temperature = 0.8):
    # initialization
    ...
    right_net = NNNet(in_features=input_features, out_features=out_features).to(device=device)
...
def test(left_net, right_net, threshold: float = -0.2):
    ...
    right_net.eval()
    with torch.no_grad():
        loop = tqdm(test_datalaoder, leave=True, total=len(test_datalaoder))
        pred_labels = []
        test_labels = []
...
if __name__ == "__main__":
    ...
    left_net, right_net = main(out_features = 8, batch_size = 128, lr = 2e-4, epochs=2, max_pairs=6000, an_ratio=0.4, temperature=0.6)
